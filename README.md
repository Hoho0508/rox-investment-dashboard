...
不想處理第三方登入時，將 `DATA_MODE=mock` 保持不變即可繼續所有介面與測試開發。正式 `DATA_MODE=live` 即使缺少 FinMind／Fugle Key，也會使用免登入的 Yahoo Finance 延遲行情與臺灣官方收盤資料；全部正式來源失效時顯示 unavailable，不會讓網站崩潰或改用 Mock。
...
台股即時／延遲價格與搜尋由 Factory 統一選擇：有 `FUGLE_MARKETDATA_API_KEY` 時先使用 Fugle；否則或單一代號失敗時依序嘗試 Yahoo Finance Chart 與 TWSE／TPEx 官方收盤資料。FinMind 只讀取 server-side 的 `process.env.FINMIND_API_TOKEN`，作為日／週／月 K 的可選來源，不再是晨報能否產生正式股票資料的必要條件。任何正式來源失敗都只會回 STALE 或 UNAVAILABLE，絕不轉 Mock。
...
- 未設定或無法連線 Fugle 時，正式站改用 Yahoo 延遲行情或 TWSE／TPEx 收盤資料；只有全部正式來源失效才顯示 unavailable。
- 已串接免費且免登入的 TWSE／TPEx、Yahoo Finance Chart／fundamentals 與 U.S. Treasury 延遲資料；尚未串接具授權保證的美股逐筆即時行情、新聞或經濟日曆。
- 預估本益比仍缺少可合法自動化的分析師一致預期來源；目前只顯示歷史／目前本益比並明確區隔。
...
下一步建議依序完成官方交易日曆、具再散布授權的分析師預估與美股即時 Provider、籌碼面歷史特徵、樣本外回測、Manual 資料管理、新聞事件去重與排程失敗通知。
