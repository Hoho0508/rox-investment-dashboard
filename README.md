...
- `mock`：不需 Key；只使用 Rox Mock Dataset，所有主要資料卡明確顯示 MOCK 與模擬警告。
- `manual`：只讀手動資料；第一版尚未完成完整輸入 UI，缺漏時顯示 unavailable，不用 Mock 補齊。
- `live`：只使用 Factory 選定的合法 Live Provider。成功資料依新鮮度標示 LIVE 或 DELAYED；請求失敗且有上一筆成功快取時標示 STALE，否則為 UNAVAILABLE。
...
Live 報告的正式跨市場輸入使用臺灣證券交易所 OpenAPI 的 TAIEX、臺灣 50、臺灣資訊科技指數，以及美國財政部 XML Feed 的 2 年／10 年公債殖利率。這些資料不需新增帳號或 Key，均標示 DELAYED、來源日期與擷取時間。TAIEX、科技指數或 US10Y 任一缺漏時，市場方向、模型信心與情境機率仍會顯示資料不足，不用固定敘事補值。
...
台股日成交價由 `src/lib/providers/finmind.ts` 與 `src/lib/market/finmind-market.ts` 透過 FinMind 取得。Token 只讀取 server-side 的 `process.env.FINMIND_API_TOKEN`。晨報的台股資料路徑不會改用 Fugle 或 Mock；缺 Token、請求失敗或資料異常時，有上一筆成功快取才標示 STALE，否則顯示 UNAVAILABLE。FinMind 日資料為盤後更新且標示 DELAYED，不冒充盤中即時行情，也不混入 Mock 基本面。

盤中即時台股由 `src/lib/market/fugle.ts` 透過 Fugle Intraday Quote 取得，金鑰只讀取 `process.env.FUGLE_MARKETDATA_API_KEY`。前端永遠只呼叫本站受登入保護的 API route，不會接觸供應商金鑰。行情提供定期與手動更新；Factory 在 Live runtime 有 Fugle Key 時選 Fugle，否則可選 FinMind 延遲收盤價。Provider 請求失敗後不會跨來源或轉 Mock；有成功快取才回 STALE，否則 UNAVAILABLE。
...
Live 串接建議優先評估 TWSE／MOPS 公開資料、公司 IR、SEC、BLS、美國財政部，以及具有明確授權條款的市場與新聞 API。FRED 與交易所指數資料可能包含 API Key、再散布、快取或 AI 使用限制，未確認授權前不接入。更換供應商時實作 `src/lib/providers/contracts.ts` 介面，並保留來源、資料日期、擷取時間、延遲狀態、信心與結構化錯誤。
...
- 已串接免費且免登入的 TWSE 指數與 U.S. Treasury 殖利率延遲資料；尚未串接美股即時行情、新聞或經濟日曆。
- 台灣官方交易日行事曆尚未接入報告排程；目前週一至週五視為交易日，國定休市日仍需官方 calendar provider。
- Mock 數值是固定測試資料，只會在明確 `DATA_MODE=mock` 時使用，不可作為投資依據。
- stale cache 目前是單一 server process 的 best-effort cache；Vercel instance 重啟或切換後可能消失，持久化 stale cache 留待 Phase 3。
