...
- 每天台灣時間 09:00 產生 `Rox Daily Morning Report`
- 首頁、每日晨報、歷史報告、核心股票、進出場評分、投資日誌、資料狀態與設定頁
- Mock／Manual／Live 三種模式的設定與安全降級（第一版完整實作 Mock；Manual 已有資料模型與驗證；Live 保留 provider 介面）
- 台灣時區與週末休市版晨報
- 同一交易日同類報告唯一索引與應用層去重
...
排程時區固定為 `Asia/Taipei`。`vercel.json` 使用 UTC cron：`0 1 * * *`，等於全年固定的台灣時間 09:00（台灣無日光節約時間）。流程會取得資料、驗證、生成、儲存並留下 `JobRun`。相同日期與 `morning` 類型由資料庫唯一索引避免重複；失敗最多重試 `REPORT_MAX_RETRIES` 次（上限 3）。
...
6. Vercel Cron 會按 `vercel.json` 每日 UTC 01:00 呼叫 endpoint。
...
- `live`：保留 Market、Fundamental、News、Macro、Currency、TaiwanInstitutional 與 EconomicCalendar provider 介面。尚未設定合法 API 或尚未實作 provider 時，不會假裝即時資料，而是安全降級。

台股日成交價由 `src/lib/providers/finmind.ts` 透過 FinMind `TaiwanStockPrice` dataset 取得。Token 只讀取 server-side 的 `process.env.FINMIND_API_TOKEN`；沒有 Token、逾時、額度不足、HTTP 錯誤、空資料或格式異常時，自動切換 Mock，並在 `error` 與資料狀態中標示原因。FinMind 日資料為盤後更新且標示延遲，不冒充盤中即時行情。
...
- 第一版沒有串接即時行情、新聞、經濟日曆或台灣官方交易日行事曆；目前週一至週五視為交易日，國定休市日仍需官方 calendar provider。
...
第二版建議依序完成官方交易日曆、合法 Live providers、Manual 資料管理、多使用者驗證、新聞事件去重、完整 CRUD，以及排程失敗通知。
